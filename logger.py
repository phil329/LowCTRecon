...
class LoggerXBase(object):
    ...
    def __init__(self, save_root, print_freq=1):
...
class TensorboardLoggerX(LoggerXBase):

    def __init__(self, save_root, print_freq=1, **kwargs):
        super().__init__(save_root, print_freq)
        if self.rank == 0:
            tb_save_dir = osp.join(save_root, 'tensorboard')
            if osp.exists(tb_save_dir): shutil.rmtree(tb_save_dir)
            os.makedirs(tb_save_dir, exist_ok=True)
            self.writer = SummaryWriter(tb_save_dir)
            if 'config' in kwargs and 'name' in kwargs:
                config = vars(kwargs['config'])
                print(config)
                for k in config:
                    if isinstance(config[k], (list, tuple)):
                        config[k] = str(config[k])
                self.writer.add_hparams(hparam_dict=config, metric_dict={}, run_name=kwargs['name'])
